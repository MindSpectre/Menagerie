...
This guide walks through building an HTTP server with `menagerie::albatross`, from a single-file minimal
...
excerpted verbatim from `examples/http-albatross/minimal_http_server.cpp` and the HTTP test suite under `tests/`,
...
`docs/architecture/albatross.md`.

## Minimal server

The full working example lives at `examples/http-albatross/minimal_http_server.cpp`. Build and run it:

```bash
$ cmake --preset debug -DBUILD_EXAMPLES=ON
$ cmake --build --preset debug --target Menagerie.Examples.Albatross.MinimalServer
$ ./build/debug/examples/http-albatross/Menagerie.Examples.Albatross.MinimalServer examples/http-albatross/server.json
```
...
`GET /users/42?v=hi` returns `user:42 v=hi` (`tests/integration_tests/component/http-albatross/test_http_tcp.cpp-47,104-109`).
...
(`tests/unit_tests/component/http-albatross/routing/test_group.cpp-80`).
...
This matches `tests/unit_tests/component/http-albatross/routing/test_middleware.cpp`: the short-circuit case is
...
(`tests/unit_tests/component/http-albatross/config/test_tls_config.cpp-18`). `min_version` is `"tls12"` (default) or
...
`component/http-albatross/types/errors/errors.hpp` has the full struct + `to_http_response` list; a user-defined
...
(`tests/integration_tests/component/http-albatross/test_http_tcp.cpp-57,132-136`). Routing misses use the same mechanism
...
no-op (`tests/integration_tests/component/http-albatross/test_http_server_lifecycle.cpp-85`). `wait_until_stopped()`
...
(`tests/integration_tests/component/http-albatross/test_http_server_lifecycle.cpp-159`, `GracefulShutdownCompletesInFlightRequests`).
