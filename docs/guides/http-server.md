...
This guide walks through building an HTTP server with `menagerie::estuary`, from a single-file minimal
...
`GET /users/42?v=hi` returns `user:42 v=hi` (`tests/integration_tests/component/http-estuary/test_http_tcp.cpp-47,104-109`).
...
(`tests/unit_tests/component/http-estuary/routing/test_group.cpp-80`).
...
This matches `tests/unit_tests/component/http-estuary/routing/test_middleware.cpp`: the short-circuit case is
...
(`tests/unit_tests/component/http-estuary/config/test_tls_config.cpp-18`). `min_version` is `"tls12"` (default) or
...
`component/http-estuary/types/errors/errors.hpp` has the full struct + `to_http_response` list; a user-defined
...
(`tests/integration_tests/component/http-estuary/test_http_tcp.cpp-57,132-136`). Routing misses use the same mechanism
...
no-op (`tests/integration_tests/component/http-estuary/test_http_server_lifecycle.cpp-85`). `wait_until_stopped()`
...
(`tests/integration_tests/component/http-estuary/test_http_server_lifecycle.cpp-159`, `GracefulShutdownCompletesInFlightRequests`).
