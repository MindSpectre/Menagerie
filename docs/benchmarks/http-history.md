# HTTP benchmark history

Archived measurements and implementation notes from July–August 2026. Claims,
source filenames, and commands below describe the experiments at that time.
See the [current HTTP benchmark guide](../../benchmarks/http-albatross/README.md)
for maintained targets and usage.

The standalone `raw_asio_probe.cpp`, `raw_asio_perthread_probe.cpp`,
`raw_asio_immediate_probe.cpp`, and `stdexec_probe.cpp` experiment programs were
removed from `benchmarks/http-albatross/` on September 18, 2026. They were not
part of the CMake targets or benchmark runners. Their original sources remain
in Git history at `f867dedb3aa1c1dbfb422e82527c7d02c690404e` under that directory; the measurements
below are retained as historical evidence, not a claim that those probes remain
...
`component/http-albatross/` never sets it. `tcp_listener.hpp` sets only `reuse_address`, and that on the *acceptor*. The h1
