...
The estuary component (`component/http-estuary/`) is Menagerie's server-side HTTP stack. Today it serves HTTP/1.1 over
...
The tree is organized as eight directories under `component/http-estuary/`, each a separate static library joined by the
...
- **`export/menagerie/estuary`**: the umbrella header consumers
  `#include`. No logic of its own; it aggregates the public leaf headers of all seven layers above. Consumers
  link `Menagerie::Component::Http` and write `#include <menagerie/estuary>`.

```text
+-----------------------------------------------------------------------+
|  export        menagerie/estuary umbrella header                         |
+-----------------------------------------------------------------------+
