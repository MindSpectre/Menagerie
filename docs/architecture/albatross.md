...
The albatross component (`component/http-albatross/`) is Menagerie's server-side HTTP stack. Today it serves HTTP/1.1 over
...
The tree is organized as eight directories under `component/http-albatross/`, each a separate static library joined by the
...
- **`export/menagerie/albatross`**: the umbrella header consumers
  `#include`. No logic of its own; it aggregates the public leaf headers of all seven layers above. Consumers
  link `Menagerie::Component::Http` and write `#include <menagerie/albatross>`.

```text
+-----------------------------------------------------------------------+
|  export        menagerie/albatross umbrella header                         |
+-----------------------------------------------------------------------+
...
- **`attach_default_listeners(Server&)`**
  ...
  `examples/http-albatross/minimal_http_server.cpp` falls back to a
