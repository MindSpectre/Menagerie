...
namespace menagerie::savanna {
    ...
    struct SqlTypeMapping<bool, Providers::PostgreSQL> {
        static constexpr std::string_view sql_type = elephant::SqlTypeRegistry::boolean;  ///< PostgreSQL SQL type-name string.
    };

    /// Maps C++ `char` to PostgreSQL's fixed single-character CHAR(1) type.
    template <>
    struct SqlTypeMapping<char, Providers::PostgreSQL> {
        static constexpr std::string_view sql_type = elephant::SqlTypeRegistry::char_type<1>();  ///< PostgreSQL SQL type-name string.
    };

    /// Maps C++ `std::int16_t` to PostgreSQL's SMALLINT type.
    template <>
    struct SqlTypeMapping<std::int16_t, Providers::PostgreSQL> {
        static constexpr std::string_view sql_type = elephant::SqlTypeRegistry::smallint;  ///< PostgreSQL SQL type-name string.
    };

    /// Maps C++ `std::int32_t` to PostgreSQL's INTEGER type.
    template <>
    struct SqlTypeMapping<std::int32_t, Providers::PostgreSQL> {
        static constexpr std::string_view sql_type = elephant::SqlTypeRegistry::integer;  ///< PostgreSQL SQL type-name string.
    };

    /// Maps C++ `std::int64_t` to PostgreSQL's BIGINT type.
    template <>
    struct SqlTypeMapping<std::int64_t, Providers::PostgreSQL> {
        static constexpr std::string_view sql_type = elephant::SqlTypeRegistry::bigint;  ///< PostgreSQL SQL type-name string.
    };
    ...
    struct SqlTypeMapping<std::uint64_t, Providers::PostgreSQL> {
        static constexpr std::string_view sql_type = elephant::SqlTypeRegistry::numeric<20, 0>();  ///< PostgreSQL SQL type-name string.
    };

    /// Maps C++ `float` to PostgreSQL's REAL (single-precision) type.
    template <>
    struct SqlTypeMapping<float, Providers::PostgreSQL> {
        static constexpr std::string_view sql_type = elephant::SqlTypeRegistry::real;  ///< PostgreSQL SQL type-name string.
    };

    /// Maps C++ `double` to PostgreSQL's DOUBLE PRECISION type.
    template <>
    struct SqlTypeMapping<double, Providers::PostgreSQL> {
        static constexpr std::string_view sql_type = elephant::SqlTypeRegistry::double_precision;  ///< PostgreSQL SQL type-name string.
    };

    /// Maps C++ `std::string` to PostgreSQL's TEXT type.
    template <>
    struct SqlTypeMapping<std::string, Providers::PostgreSQL> {
        static constexpr std::string_view sql_type = elephant::SqlTypeRegistry::text;  ///< PostgreSQL SQL type-name string.
    };
    ...
    struct SqlTypeMapping<std::vector<std::uint8_t>, Providers::PostgreSQL> {
        static constexpr std::string_view sql_type = elephant::SqlTypeRegistry::bytea;  ///< PostgreSQL SQL type-name string.
    };
...
namespace menagerie::savanna::elephant {
    ...
    constexpr std::string_view sql_type_for() {
        return savanna::sql_type_for<T, Providers::PostgreSQL>();
    }
    ...
    namespace detail {
        ...
        struct has_postgres_mapping {
            ...
            static constexpr bool value =
                std::is_same_v<T, std::monostate> || savanna::HasSqlTypeMapping<T, Providers::PostgreSQL>;
        };
    }  // namespace detail
