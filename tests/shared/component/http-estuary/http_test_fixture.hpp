...
namespace http_it {
    ...
    protected:
        ...
        menagerie::estuary::RouteRegistry registry_;
        std::vector<std::shared_ptr<menagerie::estuary::HttpController>> controllers_;
        std::optional<menagerie::estuary::Router> router_;
        std::unique_ptr<menagerie::estuary::ListenerBase> listener_;
        ...
        void add_controller(std::shared_ptr<menagerie::estuary::HttpController> ctrl) {
            menagerie::estuary::GroupBinding{registry_, controllers_, ""}.add_controller(std::move(ctrl));
        }

        /// Freeze routes, bind the listener, go live on the worker thread.
        void start(std::unique_ptr<menagerie::estuary::ListenerBase> listener) {
        ...
        std::unique_ptr<menagerie::estuary::ListenerBase> make_tcp_listener() {
            return std::make_unique<menagerie::estuary::TcpListener<menagerie::estuary::Http11Driver>>(
                std::vector{ioc_.get_executor()},
                "127.0.0.1",
                0,
                menagerie::estuary::Http11Driver{menagerie::estuary::Http11Config{}});
        }
