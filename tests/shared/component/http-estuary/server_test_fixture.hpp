...
namespace http_it {
    ...
    class PingController final : public menagerie::estuary::HttpController {
    public:
    ...
    private:
        static menagerie::estuary::AsyncResponse ping(menagerie::estuary::RequestContext ctx) {
            co_return ctx.ok("pong");
        }
        static menagerie::estuary::AsyncResponse boom(menagerie::estuary::RequestContext) {
    ...
    class LatchController final : public menagerie::estuary::HttpController {
    public:
    ...
    private:
        menagerie::estuary::AsyncResponse slow(menagerie::estuary::RequestContext ctx) {
            ...
            co_await t.async_wait(menagerie::estuary::use_strand_awaitable);
            co_return ctx.ok("slow done");
        }
        menagerie::estuary::AsyncResponse hang(menagerie::estuary::RequestContext ctx) {
    ...
    protected:
        ...
        std::vector<boost::asio::executor_work_guard<menagerie::estuary::Executor>> guards_;
        std::optional<menagerie::estuary::Server> server_;
        std::vector<std::thread> workers_;
        bool torn_down_ = false;

        void start_server(const std::function<void(menagerie::estuary::Server&)>& configure,
                          menagerie::estuary::ServerConfig cfg = menagerie::estuary::ServerConfig::Builder{}.finalize(),
                          const std::size_t io_threads      = 1) {
            std::vector<menagerie::estuary::Executor> execs;
        ...
        [[nodiscard]] menagerie::estuary::Executor bootstrap_executor() {
            auto& ctx = contexts_.emplace_back();
            guards_.emplace_back(boost::asio::make_work_guard(ctx));
