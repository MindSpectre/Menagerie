...
namespace http_it {
    ...
    class PingController final : public menagerie::albatross::HttpController {
    public:
    ...
    private:
        static menagerie::albatross::AsyncResponse ping(menagerie::albatross::RequestContext ctx) {
            co_return ctx.ok("pong");
        }
        static menagerie::albatross::AsyncResponse boom(menagerie::albatross::RequestContext) {
    ...
    class LatchController final : public menagerie::albatross::HttpController {
    public:
    ...
    private:
        menagerie::albatross::AsyncResponse slow(menagerie::albatross::RequestContext ctx) {
            ...
            co_await t.async_wait(menagerie::albatross::use_strand_awaitable);
            co_return ctx.ok("slow done");
        }
        menagerie::albatross::AsyncResponse hang(menagerie::albatross::RequestContext ctx) {
    ...
    protected:
        ...
        std::vector<boost::asio::executor_work_guard<menagerie::albatross::Executor>> guards_;
        std::optional<menagerie::albatross::Server> server_;
        std::vector<std::thread> workers_;
        bool torn_down_ = false;

        void start_server(const std::function<void(menagerie::albatross::Server&)>& configure,
                          menagerie::albatross::ServerConfig cfg = menagerie::albatross::ServerConfig::Builder{}.finalize(),
                          const std::size_t io_threads      = 1) {
            std::vector<menagerie::albatross::Executor> execs;
        ...
        [[nodiscard]] menagerie::albatross::Executor bootstrap_executor() {
            auto& ctx = contexts_.emplace_back();
            guards_.emplace_back(boost::asio::make_work_guard(ctx));
