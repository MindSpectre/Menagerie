...
namespace menagerie::test {
    ...
    protected:
        void SetUp() override {
            ...
            executor_ = std::make_unique<reef::postgres::SyncExecutor>(conn_);
            schemas_  = std::make_unique<TestSchemas>(TestSchemas::create(reef::Providers::PostgreSQL));
        }
        ...
        [[nodiscard]] reef::CompiledDynamicQuery compile_query(ExprTp&& expr) {
        ...
        void ConnectToDatabase() {
            const auto credentials = reef::postgres::ConnectionCredentials::Builder{}
        ...
        void CreateUsersTable() const {
            const auto result = executor_->execute(std::string(SchemaDDL::users_table(reef::Providers::PostgreSQL)));
            ASSERT_TRUE(result.is_success()) << "Failed to create users table";
        }

        void CreateUsersExtendedTable() const {
            const auto result =
                executor_->execute(std::string(SchemaDDL::users_extended_table(reef::Providers::PostgreSQL)));
            ASSERT_TRUE(result.is_success()) << "Failed to create users_extended table";
        }

        void CreatePostsTable() const {
            const auto result = executor_->execute(std::string(SchemaDDL::posts_table(reef::Providers::PostgreSQL)));
            ASSERT_TRUE(result.is_success()) << "Failed to create posts table";
        }

        void CreateOrdersTable() const {
            const auto result = executor_->execute(std::string(SchemaDDL::orders_table(reef::Providers::PostgreSQL)));
            ASSERT_TRUE(result.is_success()) << "Failed to create orders table";
        }

        void CreateOrdersExtendedTable() const {
            const auto result =
                executor_->execute(std::string(SchemaDDL::orders_extended_table(reef::Providers::PostgreSQL)));
            ASSERT_TRUE(result.is_success()) << "Failed to create orders_extended table";
        }

        void CreateCommentsTable() const {
            const auto result = executor_->execute(std::string(SchemaDDL::comments_table(reef::Providers::PostgreSQL)));
        ...
        [[nodiscard]] reef::postgres::SyncExecutor& executor() const {
            return *executor_;
        }
        ...
        std::unique_ptr<reef::postgres::SyncExecutor> executor_;
        std::unique_ptr<TestSchemas> schemas_;
        reef::QueryCompiler<reef::postgres::PostgresDialect, reef::ParamMode::Inline> compiler_{};
    };
