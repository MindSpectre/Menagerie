...
DROP TABLE IF EXISTS users;
    ...
    std::string_view SchemaDDL::users_table(savanna::Providers dialect) {
        switch (dialect) {
            case savanna::Providers::PostgreSQL:
    ...
    std::string_view SchemaDDL::users_extended_table(savanna::Providers dialect) {
        switch (dialect) {
            case savanna::Providers::PostgreSQL:
    ...
    std::string_view SchemaDDL::posts_table(savanna::Providers dialect) {
        switch (dialect) {
            case savanna::Providers::PostgreSQL:
    ...
    std::string_view SchemaDDL::orders_table(savanna::Providers dialect) {
        switch (dialect) {
            case savanna::Providers::PostgreSQL:
    ...
    std::string_view SchemaDDL::orders_extended_table(savanna::Providers dialect) {
        switch (dialect) {
            case savanna::Providers::PostgreSQL:
    ...
    std::string_view SchemaDDL::comments_table(savanna::Providers dialect) {
        switch (dialect) {
            case savanna::Providers::PostgreSQL:
    ...
    std::string_view SchemaDDL::drop_all(savanna::Providers dialect) {
        switch (dialect) {
            case savanna::Providers::PostgreSQL:
    ...
    TestSchemas TestSchemas::create(savanna::Providers provider) {
        // Build a DynamicTable for Record-based tests
        auto users_dyn = std::make_shared<savanna::DynamicTable>("users", provider);
