...
protected:
    ...
    void CleanupTestTables() const {
        std::ignore = executor().execute("DROP TABLE IF EXISTS ddl_orders_test CASCADE");
        std::ignore = executor().execute("DROP TABLE IF EXISTS ddl_constraints_test CASCADE");
        std::ignore = executor().execute("DROP TABLE IF EXISTS ddl_settings_test CASCADE");
        std::ignore = executor().execute("DROP TABLE IF EXISTS ddl_temp_table CASCADE");
        std::ignore = executor().execute("DROP TABLE IF EXISTS ddl_test_table CASCADE");
    }
    ...
    [[nodiscard]] bool TableExists(const std::string& table_name) const {
        auto result = executor().execute("SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = '" +
                                         table_name + "')");
        if (result.has_value() && result.value().rows() > 0) {
...
TEST_F(CompiledDdlTest, CreateTableBasicExecutes) {
    ...
    std::ignore = executor().execute("DROP TABLE IF EXISTS users CASCADE");

    auto query  = compile_query(create_table(schemas().users_dynamic));
    auto result = executor().execute(query);

    ASSERT_TRUE(result.has_value()) << "CREATE TABLE failed: " << result.error();
...
TEST_F(CompiledDdlTest, CreateTableIfNotExistsDoesNotFail) {
    ...
    ASSERT_TRUE(result1.has_value()) << "First CREATE failed: " << result1.error();
    ...
    ASSERT_TRUE(result2.has_value()) << "Second CREATE should not fail with IF NOT EXISTS";
...
TEST_F(CompiledDdlTest, CreateTableWithConstraintsExecutes) {
    ...
    EXPECT_TRUE(insert_result.has_value()) << "INSERT failed: " << insert_result.error();
    ...
    EXPECT_FALSE(dup_result.has_value()) << "Duplicate email should violate UNIQUE constraint";
}
...
TEST_F(CompiledDdlTest, CreateTableWithForeignKeyExecutes) {
    ...
    EXPECT_TRUE(insert_result.has_value()) << "INSERT with valid FK failed";
    ...
    EXPECT_FALSE(invalid_result.has_value()) << "Invalid FK should be rejected";
...
TEST_F(CompiledDdlTest, CreateTableWithDefaultExecutes) {
    ...
    EXPECT_TRUE(insert_result.has_value()) << "INSERT with defaults failed";
...
TEST_F(CompiledDdlTest, DropTableBasicExecutes) {
    ...
    ASSERT_TRUE(result.has_value()) << "DROP TABLE failed: " << result.error();
...
TEST_F(CompiledDdlTest, DropTableIfExistsDoesNotFail) {
    ...
    ASSERT_TRUE(result.has_value()) << "DROP TABLE IF EXISTS should not fail on non-existent table";
}
...
TEST_F(CompiledDdlTest, DropTableCascadeExecutes) {
    ...
    ASSERT_TRUE(create_child.has_value());

    // Insert data
    InsertTestUsers();
    auto insert_child = executor().execute("INSERT INTO child_table (user_id) VALUES (1)");
    ASSERT_TRUE(insert_child.has_value());

    // DROP CASCADE should work even with dependent table
    auto query  = compile_query(drop_table(schemas().users_dynamic, false, true));
    auto result = executor().execute(query);

    ASSERT_TRUE(result.has_value()) << "DROP TABLE CASCADE failed: " << result.error();
    EXPECT_FALSE(TableExists("users"));

    // Cleanup child table
    std::ignore = executor().execute("DROP TABLE IF EXISTS child_table CASCADE");
}
...
TEST_F(CompiledDdlTest, DropTableIfExistsCascadeExecutes) {
    ...
    ASSERT_TRUE(create_dep.has_value());

    auto query  = compile_query(drop_table(schemas().users_dynamic, true, true));
    auto result = executor().execute(query);

    ASSERT_TRUE(result.has_value()) << "DROP TABLE IF EXISTS CASCADE failed";
    EXPECT_FALSE(TableExists("users"));

    // Cleanup
    std::ignore = executor().execute("DROP TABLE IF EXISTS dependent_table CASCADE");
}
...
TEST_F(CompiledDdlTest, DropTableByNameExecutes) {
    ...
    ASSERT_TRUE(result.has_value()) << "DROP TABLE by name failed: " << result.error();
...
TEST_F(CompiledDdlTest, CreateInsertSelectDropLifecycle) {
    ...
    ASSERT_TRUE(create_result.has_value()) << "CREATE failed: " << create_result.error();
    ...
    ASSERT_TRUE(insert_result.has_value()) << "INSERT failed";
    ...
    ASSERT_TRUE(drop_result.has_value()) << "DROP failed: " << drop_result.error();
...
TEST_F(CompiledDdlTest, CreateTableWithAllConstraintTypes) {
    ...
    // Cleanup
    std::ignore = executor().execute("DROP TABLE IF EXISTS ddl_comprehensive_test CASCADE");
}
