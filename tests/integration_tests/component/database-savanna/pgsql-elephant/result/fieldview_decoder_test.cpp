...
TEST_F(FieldViewDecoderTest, NullAsIntThrows) {
    ...
    auto row = result.value().get_row(0);
    auto fv  = row.at(0);
    EXPECT_THROW((void)fv.as<int32_t>(), menagerie::savanna::null_conversion_error);
}
...
TEST_F(FieldViewDecoderTest, RoundTripAllTypesViaTable) {
    ...
    ASSERT_TRUE(text_result.has_value()) << text_result.error().format();
    ...
    ASSERT_TRUE(bin_result.has_value()) << bin_result.error().format();
