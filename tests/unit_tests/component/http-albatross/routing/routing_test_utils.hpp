...
namespace http_routing_test {
    ...
    template <typename T>
    T run_awaitable(boost::asio::awaitable<T, menagerie::albatross::Strand> aw) {
    ...
    protected:
        ...
        menagerie::albatross::Request make_request(const menagerie::albatross::HttpMethod m, std::string target) {
        ...
        menagerie::albatross::RequestContext make_ctx(const menagerie::albatross::HttpMethod m, std::string target) {
            return menagerie::albatross::RequestContext{make_request(m, std::move(target)), alloc_};
        }
    };
