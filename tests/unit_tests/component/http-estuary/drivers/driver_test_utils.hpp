...
namespace http_driver_test {
    ...
    public:
        ...
        boost::asio::awaitable<void, menagerie::estuary::Strand> async_close() {
            stream_.close();
            co_return;
        }
        ...
        [[nodiscard]] static menagerie::estuary::Protocol negotiated_protocol() noexcept {
            return menagerie::estuary::Protocol::http1;
        }
    ...
    private:
        stream_type stream_;
        menagerie::estuary::RequestArena arena_;
        boost::asio::cancellation_signal signal_;
    };

    static_assert(menagerie::estuary::IsStreamConnection<TestConnection>);
    ...
    std::vector<ParsedResponse> exchange(Driver& driver,
                                         menagerie::estuary::Router& router,
